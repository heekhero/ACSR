...
from utils import NShotTaskSamplerFromArray, accuracy, LabelSmoothingCrossEntropy, concat_all_gather
...
def main():
    ...
    with open(base_feature_path, 'rb') as f:
        ...
        #shuffle the base features to increase randomness when spliting
        shuffle_indices = torch.randperm(frozen_baseset_embeddings.size(0)).cuda(non_blocking=True)

        torch.distributed.broadcast(shuffle_indices, src=0)
        shuffle_indices = shuffle_indices.cpu()


        print('Rank : {}  Pre Shuffle Indices : {}'.format(args.local_rank, shuffle_indices[:5]))
        frozen_baseset_embeddings = frozen_baseset_embeddings[shuffle_indices]
    ...
    now_dict.update(store_dict_no_fc)
    ...
    #randomly divide the base set into M=dist_size (by default 4) disjoint subsets
    baseset = dataset_class(subset='base', state='center+id', image_size=args.image_size, local_rank=args.local_rank, world_size=dist_size, shuffle_indices=shuffle_indices)
    ...
    for task_index, (images, indices) in enumerate(taskloader):
        ...
        #synchronize data from different models
        torch.distributed.broadcast(images, src=0)
        ...
        if task_index < 2:
            print(args.local_rank, train_indices)

        if task_index == 0 and args.local_rank == 1:
            for tid in indices:
        ...
        for name, p in finetune_model.named_parameters():
            if task_index == 0 and args.local_rank == (GLOBAL_CNT % dist_size):
        ...
        for bid in range(args.epochs_finetune):
            for batch_data in fs_loader_novel:
                ...
                with torch.no_grad():
                    aux_embeddings = frozen_baseset_embeddings[aux_indices+baseset.index_begin].data

                finetune_model.zero_grad()

                _, finetune_logits = finetune_model(finetune_images)
                finetune_loss = finetune_criterion(finetune_logits, finetune_labels)

                finetune_base_embeds, _ = finetune_model(aux_images)

                aux_loss = (F.normalize(finetune_base_embeds, dim=-1) * F.normalize(aux_embeddings, dim=-1)).sum(dim=1).mean()
                ...
                if args.local_rank == (GLOBAL_CNT % dist_size):
                    print('\rfinetune   epoch : {}      finetune_loss : {:.4f}      aux loss : {:.4f}'.format(bid, finetune_loss.item(), aux_loss.item()), end='')
                GLOBAL_CNT += 1

        with torch.no_grad():
            finetune_model.eval()
            test_probs = torch.softmax(finetune_model(test_images)[1], dim=-1)
            gather_test_probs = concat_all_gather(test_probs)
            if args.local_rank == 1 and task_index < 2:
                print(gather_test_probs.size())

            # average results from different ensemble models to generate recognition results of the ensemble system
            ensemble_test_probs = torch.mean(gather_test_probs, dim=0)
            acc_lcs = accuracy(ensemble_test_probs, test_labels)
